...
const CollectPoint = () => {
  ...
  useEffect(() => {
    async function loadPosition() {
      ...
      if (status !== 'granted') {
        Alert.alert('Ops....', 'Precisamos da permissão de sua localização.')
        return
      }
  ...
  useEffect(() => {
    ...
    api.get('collect-points', options).then((resp) => {
      if (resp && resp.data) {
        setCollectPoints(resp.data)
      }
    })
  }, [selectedItems])
          ...
          {initialPos[0] !== 0 && (
            <MapView style={styles.map} initialRegion={{ latitude: initialPos[0], longitude: initialPos[1], latitudeDelta: 0.014, longitudeDelta: 0.014 }}>
              {collectPoints.map((point) => (
                <Marker key={String(point.id)} style={styles.mapMarker} coordinate={{ latitude: point.latitude, longitude: point.longitude }} onPress={() => { handleNavigationDetail(point.id) }}>
